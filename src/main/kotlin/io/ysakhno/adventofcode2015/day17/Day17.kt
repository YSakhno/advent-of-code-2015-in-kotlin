 ...
 */
package io.ysakhno.adventofcode2015.day17
...
private fun List<Int>.generateSubsets() = sequence {
    for (i in 1..<(1 shl size)) {
        yield(filterIndexed { idx, _ -> i and (1 shl idx) != 0 })
    }
}

private fun part1(input: List<String>, totalCapacity: Int) =
    input.map(String::toInt).generateSubsets().count { it.sum() == totalCapacity }

private fun part2(input: List<String>, totalCapacity: Int) = input.map(String::toInt)
    .generateSubsets()
    .filter { it.sum() == totalCapacity }
    .groupingBy(List<Int>::size)
    .eachCount()
    .run { getValue(keys.min()) }

fun main() {
    // Test if implementation meets criteria from the description
    val testInput = problemInput.readTest()
    assertEquals(4, part1(testInput, 25), "Part one (sample input)")
    assertEquals(3, part2(testInput, 25), "Part two (sample input)")
