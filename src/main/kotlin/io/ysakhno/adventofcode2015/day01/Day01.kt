 ...
 */
package io.ysakhno.adventofcode2015.day01
...
private fun List<String>.toInstructions() = filterNot(String::isBlank).map(String::trim).first()
...
private fun part2(input: List<String>) = input.toInstructions().fold(0 to 1) { (floor, idx), ins ->
    if (floor == -1) floor to idx else (if (ins == ')') floor - 1 else floor + 1) to idx + 1
}.second - 1

fun main() {
    // Test if implementation meets criteria from the description
    val testInput = problemInput.readTest()
    assertEquals(3, part1(testInput), "Part one (sample input)")
    assertEquals(1, part2(testInput), "Part two (sample input)")
