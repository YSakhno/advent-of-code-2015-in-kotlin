 ...
 * of characters of code in each original string literal.  For example, for the strings above, the total encoded length
 ...
 */
package io.ysakhno.adventofcode2015.day08
...
private fun computeOverhead(line: String): Int {
    val string = line.substring(1, line.length - 1)
...
private fun part1(input: List<String>) = input.sumOf(::computeOverhead)

private fun computeUnderhead(line: String) =
    line.sumOf { if (it == '\\' || it == '"') 2 else 1L }.toInt() - line.length + 2

private fun part2(input: List<String>) = input.sumOf(::computeUnderhead)

fun main() {
    // Test if implementation meets criteria from the description
    val testInput = problemInput.readTest()
    assertEquals(12, part1(testInput), "Part one (sample input)")
    assertEquals(19, part2(testInput), "Part two (sample input)")
