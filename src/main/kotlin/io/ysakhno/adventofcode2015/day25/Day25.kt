 ...
 * right.  This process repeats until the infinite paper is covered.  So, the first few codes are filled in this order:
 *
 *      | 1   2   3   4   5   6
 *   ---+---+---+---+---+---+---+
 *    1 |  1   3   6  10  15  21
 ...
 * machine?
 */
package io.ysakhno.adventofcode2015.day25
...
private fun part1(input: String): Int {
    val (row, column) = input.allInts().toList()
    val triRow = row + column - 2
    val triNum = (1 + triRow) * triRow / 2 + column

    return generateSequence(20151125L) { it * 252533 % 33554393 }.drop(triNum - 1).first().toInt()
}

fun main() {
    // Test if implementation meets criteria from the description
    val testInput = problemInput.readTest().first()
    assertEquals(27995004, part1(testInput), "Part one (sample input)")
